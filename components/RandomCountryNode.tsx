...
type RandomCountryNodeData = NodeData & {
  regenerateTimestamp?: number;
};

export default function RandomCountryNode({ id, data, selected, type }: NodeProps<Node<RandomCountryNodeData>>) {
  ...
  useEffect(() => {
    const cached = wordListCache.get('countries');
    if (cached) {
      setWords(cached);
      setLoading(false);
      return;
    ...
    fetch('/textubes/wordlists/countries.json')
      ...
      .then((data: string[]) => {
        wordListCache.set('countries', data);
        setWords(data);
        setLoading(false);
      })
      .catch(err => {
  ...
  // Generate initial random country when words are loaded
  useEffect(() => {
    ...
    if (!data.value) {
      const randomCountry = words[Math.floor(Math.random() * words.length)];
      updateNodeData(id, { value: randomCountry });
    }
