...
type RandomInterjectionNodeData = NodeData & {
  regenerateTimestamp?: number;
};

export default function RandomInterjectionNode({ id, data, selected, type }: NodeProps<Node<RandomInterjectionNodeData>>) {
  ...
  useEffect(() => {
    const cached = wordListCache.get('interjections');
    if (cached) {
      setWords(cached);
      setLoading(false);
      return;
    ...
    fetch('/textubes/wordlists/interjections.json')
      ...
      .then((data: string[]) => {
        wordListCache.set('interjections', data);
        setWords(data);
        setLoading(false);
      })
      .catch(err => {
        console.error('Error loading interjections:', err);
        setError('Failed to load word list');
        setLoading(false);
      });
  }, []);

  // Generate initial random interjection when words are loaded
  useEffect(() => {
    ...
    if (!data.value) {
      const randomInterjection = words[Math.floor(Math.random() * words.length)];
      updateNodeData(id, { value: randomInterjection });
    }
    ...
    <div className={`node-help-wrapper ${data.helpActive ? 'help-active' : ''}`}>
      ...
      >
        <div className="node-description">
          Generates a random interjection
        </div>
